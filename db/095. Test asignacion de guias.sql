...
IF NOT EXISTS (SELECT 1 FROM parque.AreaProtegida WHERE ID = @IDParque)
	EXEC parque.AreaProtegidaAlta @ID = @IDParque, @TipoArea = 'Parque Nacional', @Nombre = 'Parque Test Guias', @Superficie = 1000;

IF NOT EXISTS (SELECT 1 FROM actividad.TipoActividad WHERE Nombre = 'Tour Guiado Test')
	EXEC actividad.TipoActividadAlta @Nombre = 'Tour Guiado Test';
...
IF NOT EXISTS (SELECT 1 FROM actividad.Actividad WHERE ID_AreaProtegida = @IDParque AND Nombre = 'Mirador Test')
	EXEC actividad.ActividadAlta @ID_AreaProtegida = @IDParque, @ID_TipoActividad = @IDTipoActividad, @Nombre = 'Mirador Test', @Duracion = 90, @Costo = 0, @CupoMaximo = 10;
...
IF NOT EXISTS (SELECT 1 FROM personal.GuiaAutorizado WHERE CUIL = @CUILGuia)
	EXEC personal.GuiaAutorizadoAlta @CUIL = @CUILGuia, @Nombre = 'Guia', @Apellido = 'Test', @Autorizado = 1;
ELSE
	EXEC personal.GuiaAutorizadoModificacion @CUIL = @CUILGuia, @Autorizado = 1;

IF NOT EXISTS (SELECT 1 FROM personal.HabilitacionGuia WHERE Nombre = 'Habilitacion Test')
	EXEC personal.HabilitacionGuiaAlta @Nombre = 'Habilitacion Test', @Descripcion = 'Habilitacion de prueba';
...
IF NOT EXISTS (SELECT 1 FROM personal.GuiaConHabilitacion WHERE CUIL_GuiaAutorizado = @CUILGuia AND ID_HabilitacionGuia = @IDHabilitacion)
	EXEC personal.GuiaConHabilitacionAlta @CUIL_GuiaAutorizado = @CUILGuia, @ID_HabilitacionGuia = @IDHabilitacion, @FechaObtenido = '2025-01-01', @FechaExpiracion = '2027-12-31';

IF NOT EXISTS (SELECT 1 FROM personal.PermisoDeTrabajo WHERE CUIL_GuiaAutorizado = @CUILGuia AND ID_AreaProtegida = @IDParque)
	EXEC personal.PermisoDeTrabajoAlta @ID_AreaProtegida = @IDParque, @CUIL_GuiaAutorizado = @CUILGuia, @FechaInicio = '2025-01-01', @FechaFin = '2027-12-31';

IF EXISTS (SELECT 1 FROM actividad.GuiaAsignadoTour WHERE ID_Actividad = @IDActividad AND CUIL_GuiaAutorizado = @CUILGuia)
	EXEC actividad.GuiaAsignadoTourBaja @ID_Actividad = @IDActividad, @CUIL_GuiaAutorizado = @CUILGuia;

PRINT('Caso exitoso: asignacion de guia');
EXEC actividad.GuiaActividadAsignar
